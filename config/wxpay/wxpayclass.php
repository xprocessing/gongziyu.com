...
CLASS WXHongBao {
  ...
  private $mch_id = "1510223641"; //商户ID写死
  //private $sub_mch_id = "1510223641";    //微信支付分配的子商户号，服务商模式下必填           
  private $wxappid = "wx7159d3df9cfb4c0e";          //微信公众号，写死
  //private $msgappid = "wx7159d3df9cfb4c0e";          //务商模式下触达用户时的appid(可填服务商自己的appid或子商户的appid)
  private $client_ip = "121.42.135.19"; //"127.0.0.1"; //调用红包接口的主机的IP,服务端IP,写死，即脚本文件所在的IP
  //private $client_ip = $_SERVER['SERVER_ADDR'];
  private $apikey = "gongziyu0709gongziyu0709gongziyu";    //pay的秘钥值
  private $total_num = 1;                   //发放人数。固定值1，不可修改
  private $nick_name = "公子鱼";           //红包商户名称
  private $send_name = "返现红包";          //红包派发者名称
  ...
  function __construct(){
    ...
    $this->apiclient_cert = getcwd() . "/wxpaycert/apiclient_cert.pem";
    $this->apiclient_key = getcwd() . "/wxpaycert/apiclient_key.pem";
    //$this->apiclient_ca = getcwd() . "/wxpaycert/rootca.pem";
  }
  ...
  //echo $xml;
    ...
    //debug
    file_put_contents("hbxml.debug.txt",$xml);
  ...
  private function gen_Sign(){
    ...
    //$param["sub_mch_id"]=$this->$sub_mch_id;    
    ...
    //可以用下面方法查看
    file_put_contents("sign.debug.txt",$sign_raw);//debug
...
echo $sign_raw;
  ...
  public function genXMLParam(){
    ...
    // <msgappid>".$this->msgappid."</msgappid>     
   $xml = "<xml>
         <act_name><![CDATA[".$this->act_name."]]></act_name> 
         <client_ip><![CDATA[".$this->client_ip."]]></client_ip> 
         <max_value>".$this->max_value."</max_value> 
